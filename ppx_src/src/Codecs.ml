...
let rec parameterizeCodecs typeArgs encoderFunc decoderFunc generatorSettings
  ...
  let (subEncoders, subDecoders) =
    (typeArgs |>
       (List.map
          (fun core_type -> generateCodecs generatorSettings core_type)))
      |> List.split in
...
and generateConstrCodecs { doEncode; doDecode }
  ...
  let open Longident in
    ...
    | ((Ldot
        (((Ldot
         (((Lident ("Belt"))[@explicit_arity ]), "Result"))[@explicit_arity ]),
         "t"))[@explicit_arity ])
        ->
        (((match doEncode with
    ...
    | ((Ldot
        (((Ldot
         (((Lident ("Js"))[@explicit_arity ]), "Dict"))[@explicit_arity ]),
         "t"))[@explicit_arity ])
        ->
        (((match doEncode with
    ...
    | ((Ldot
        (((Ldot
         (((Lident ("Js"))[@explicit_arity ]), "Json"))[@explicit_arity ]),
         "t"))[@explicit_arity ])
        ->
        (((match doEncode with
          ...
          ((match doDecode with
            | true ->
                ((Some
                    (([%expr
                        (fun v -> ((Belt.Result.Ok (v))[@explicit_arity ]))])))
                [@explicit_arity ])
            | false -> None)))
  ...
  | ((Ptyp_constr (constr, typeArgs))[@explicit_arity ]) ->
      ...
      let (encode, decode) =
        ...
        | ((Ok (((Some (attribute))[@explicit_arity ])))[@explicit_arity ])
            ...
            (((match doEncode with
               | true -> ((Some (([%expr let (e, _) = [%e expr] in e])))
                   [@explicit_arity ])
               | false -> None)),
              ((match doDecode with
                | true -> ((Some (([%expr let (_, d) = [%e expr] in d])))
                    [@explicit_arity ])
                | false -> None)))
        | ((Error (s))[@explicit_arity ]) -> fail ptyp_loc s in
