...
let generateEncoderCase encodeDecodeFlags unboxed row =
  ...
  | ((Rtag ({txt = name; loc}, _attributes, coreTypes)) [@explicit_arity]) ->
    ...
    let lhsVars =
      ...
      | _ ->
        args
        |> List.mapi (fun i _ -> mkloc ("v" ^ string_of_int i) loc |> Pat.var)
        |> Pat.tuple
    ...
    in
    {
      ...
      pc_rhs =
        [%expr
          ...
          | true ->
            Decco.error "Invalid number of arguments to polyvariant constructor"
              v
          | false -> [%e decoded]];
    }
  ...
  | ((Rtag ({txt; loc}, _, args)) [@explicit_arity]) -> (
    ...
    | a :: [] -> (
      ...
      | ((Some d) [@explicit_arity]) ->
        let constructor =
          Exp.construct (lid txt) (Some [%expr v] [@explicit_arity])
        in
        (Some
           [%expr
