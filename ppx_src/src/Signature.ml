...
let rec addDecoderParams paramNames resultType =
  ...
  | hd::tl ->
      let decoderParam =
        [%type : Js.Json.t -> [%t makeResultType (Ast_helper.Typ.var hd)]] in
...
let generateSigDecls { doEncode; doDecode } typeName paramNames =
  ...
  let valueType =
    (paramNames |> (List.map Ast_helper.Typ.var)) |>
      (Ast_helper.Typ.constr (lid typeName)) in
  let decls = [] in
  let decls =
    match doEncode with
    | true ->
...
let mapTypeDecl decl =
  let { ptype_attributes; ptype_name = { txt = typeName }; ptype_params;
        ptype_loc }
    = decl in
