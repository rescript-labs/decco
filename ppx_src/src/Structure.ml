  ...
  List.fold_right
    (fun s ->
       fun acc ->
         ...
         Exp.fun_ Asttypes.Nolabel None pat acc) paramNames
    ([%expr fun v -> ([%e expr]) v])
let generateCodecDecls typeName paramNames (encoder, decoder) =
  ...
  let vbs =
    ...
    | ((Some (encoder))[@explicit_arity ]) ->
        vbs @
          [Vb.mk ~attrs:[attrWarning ([%expr "-39"])] encoderPat
             (addParams encoderParamNames encoder)] in
  let vbs =
    match decoder with
    | None -> vbs
    | ((Some (decoder))[@explicit_arity ]) ->
        vbs @
          [Vb.mk
             ...
             decoderPat (addParams decoderParamNames decoder)] in
  vbs
let mapTypeDecl decl =
  let { ptype_attributes; ptype_name = { txt = typeName }; ptype_manifest;
        ptype_params; ptype_loc; ptype_kind }
    = decl in
  let isUnboxed =
      ...
      (match (ptype_manifest, ptype_kind) with
       ...
       | (((Some
          ...
          Ptype_abstract) ->
           ...
           generateCodecDecls typeName (getParamNames ptype_params)
