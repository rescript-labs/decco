...
let generateEncoderCase generatorSettings unboxed
  ...
  | Pcstr_record _ ->
      fail pcd_loc "This syntax is not yet implemented by decco"
  ...
  | ((Pcstr_tuple (args))[@explicit_arity ]) ->
      ...
      let decoded =
        ...
        | [] ->
            ...
            [%expr
              ((Belt.Result.Ok (([%e Exp.construct ident None])))
                [@explicit_arity ])]
        | _ -> generateArgDecoder generatorSettings args name in
      {
        pc_lhs =
        ...
        pc_rhs =
          ([%expr
             (match (Js.Array.length tagged) != ([%e argLen]) with
              | true ->
                  Decco.error
                    "Invalid number of arguments to variant constructor" v
              | false -> [%e decoded])])
      }
  | Pcstr_record _ ->
      fail pcd_loc "This syntax is not yet implemented by decco"
       ...
       | a::[] ->
           ...
           (match d with
            | ((Some (d))[@explicit_arity ]) ->
                let constructor =
                  Exp.construct (lid name) ((Some (([%expr v])))
                    [@explicit_arity ]) in
                ((Some
                    (([%expr
                        (fun v ->
...
let generateCodecs ({ doEncode; doDecode } as generatorSettings) constrDecls
  ...
  let encoder =
    match doEncode with
    | true ->
        (((List.map (generateEncoderCase generatorSettings unboxed)
             constrDecls)
            |> (Exp.match_ ([%expr v])))
                 ...
                 (([%expr
                     (fun v ->
                        ...
                        | ((Js.Json.JSONArray ([||]))[@explicit_arity ]) ->
                            Decco.error "Expected variant, found empty array"
                              v
