...
describe("bool", () => {
  test("b_encode", () => b_encode(true) |> Js.Json.classify |> expect |> toBe(Js.Json.JSONTrue))
...
describe("array", () => {
  ...
  describe("a_decode", () => {
    let json = ["10", "20"] |> Js.Array.map(Js.Json.string) |> Js.Json.array
...
describe("option", () => {
  describe("o_encode", () => {
    test("none", () =>
      o_encode(s_encode, None) |> Js.Json.classify |> expect |> toBe(Js.Json.JSONNull)
    )
...
describe("falseable", () => {
  describe("falseable_encode", () => {
    test("none", () =>
      falseable_encode(s_encode, None) |> Js.Json.classify |> expect |> toBe(Js.Json.JSONFalse)
    )
